...
- 部屋は**パステルの空**（水色→ピンク、雲がゆっくり流れる）。
  **7スロット制**: 人間＋ローカルボットで常に7体。全員が画面下部の帯を
  小さくうろうろして待機する（自分は大きめ・右寄りの固定エリア）
- 会話は **LINE型の発話フィード**。発話ごとに「ミニキャラ＋吹き出し」の行が
  下から積まれ（自分は右寄せ・他は左寄せ）、220msのリズムで1文字ずつ現れる
  （口パク＋パ音つき）。古い行は押し上げられ、しばらくすると薄れて消える。
  発話中は下部のその子もうろうろを止めて口パクする
- **表情リアクション**: 投稿の原文を**端末内だけ**でキーワード判定し、
  挨拶=おじぎ・喜び=弾む・通常=そのまま。DBへ送るのは pagyo と
  リアクション種別のみ。新しいリアクションの初獲得時は自分がキラッと光る
  （獲得はローカルに永続化）
- **ログイン = ふわっと登場 / ログアウト（onDisconnect）= その場で薄れて消える**。
  沈黙は自由（何もしなくても何も起きない）
- v1 の遺産（のろし・星）は `src/legacy/RoomV1Noroshi.tsx`、
  v2 の遺産（レーン漂流・追従吹き出し）は `src/legacy/RoomV2Sky.tsx` に
  未使用のまま保全（動作する統合は main / room-v2 ブランチ）
...
- ルーム画面 room-v3（パステルの空・LINE型発話フィード・下部うろうろ・
  表情リアクション3種）
- 音（パ行ボイス・220msリズム・iOSアンロック対応）
- Firebase 同期（在室7人制限・投稿は変換後のパ行＋リアクション種別のみ・
  ボットはローカル）
...
- リアクションのキーワード辞書の拡充と種類の追加
  （現状は挨拶/喜び/通常の3種・部分一致のみ）
- ボットのスロット埋めが全員プペの台詞・挙動を共有している。
  個体ごとの台詞・性格のバリエーションを付ける
- 実機での体感調整（うろうろ速度・フィードの行数/保持時間・吹き出しサイズ）
...
**変換後のパ行文字列（pagyo）と、端末内キーワード判定によるリアクション種別のみ**。
変換前の原文は、いかなる形でも送信・保存しない。`src/core/room.ts` の
`post()` が受け取るのは pagyo と reaction だけであり、原文がネットワークへ
到達する経路をコード上作らないこと（キーワード判定 `src/core/reaction.ts` の
`detectReaction()` も原文を外に出さない）。
...
src/
  core/        … UIから独立したロジック層
    ...
    reaction.ts    表情リアクションの端末内キーワード判定と獲得記録
  ...
  components/  … UI コンポーネント
    ...
    WanderingPapi.tsx  下部を左右にさまようパピ（正弦2波合成・paused で停止・
                       animClass でリアクション演出を重ねる）
    HoveringPapi.tsx  v1の定位置ホバリングパピ（room-v3 では未使用。
                      legacy/RoomV1Noroshi.tsx が参照）
  legacy/      … 旧バージョンの遺産（未使用のまま保全）
    RoomV1Noroshi.tsx  v1: 夜空・のろしS字カーブ・星の蓄積（main ブランチに統合あり）
    RoomV2Sky.tsx      v2: レーン漂流・キャラ追従吹き出し（room-v2 ブランチに統合あり）
  App.tsx      … ルーム画面 room-v3。LINE型発話フィード・下部うろうろ・
                 表情リアクション（入力欄内で変身 → 行が下から積まれ
                 1文字ずつ＋口パク＋パ音）
public/
