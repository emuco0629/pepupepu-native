/**
 * 【v2 ルーム画面の保全コピー — room-v3 では未使用】
 *
 * パステルの空・7レーンの横漂流・キャラ追従吹き出しによる v2 ルーム画面。
 * room-v3（LINE型フィード・下部うろうろ）への置き換えにあたり、
 * v2 のコードを削除せず未使用のまま残すため、旧 src/App.tsx を
 * そのままここへ移した（import パスのみ調整）。どこからも import されない。
 * 動作する統合は room-v2 ブランチにある。
 * 関連する未使用CSS: index.css の .field / .chara / .bubble
 */
